...
(function() {
    ...
    function disableToolUsage() {
        ...
        // Use event delegation on document instead of attaching to each element
        document.addEventListener('click', function(e) {
            ...
            // Skip if clicking inside any modal
            if (el.closest('.modal') ||
                el.closest('.example-modal') ||
                el.closest('.preview-modal') ||
            ...
            // Skip navigation
            if (el.closest('.nav') || el.closest('nav') || el.closest('.navbar') || el.closest('.sidebar')) {
                return;
            }
            
            // Skip example-related buttons
            if (el.classList.contains('example-btn') ||
                ...
                el.id?.toLowerCase().includes('example') ||
                el.id?.toLowerCase().includes('close')) {
                return;
            }
            
            // Check if this is an interactive element that should trigger upgrade
            const isFormElement = el.matches('input, textarea, select');
            const isSubmitButton = el.matches('button[type="submit"]');
            const isActionButton = el.matches('button') && 
                                   !el.textContent?.toLowerCase().includes('example') &&
                                   !el.textContent?.toLowerCase().includes('close') &&
                                   !el.textContent?.toLowerCase().includes('cancel') &&
                                   !el.textContent?.toLowerCase().includes('preview');
            
            // Only block form submissions and action buttons, not navigation/modal buttons
            if (isSubmitButton || (isActionButton && el.closest('form'))) {
                e.preventDefault();
                e.stopPropagation();
